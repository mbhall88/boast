...
- **ADR-0008's "operational caveats do not belong in prose" still stands.** The split is by Outcome kind, not by content: a `Metric.note` on a real Value is a Notice and travels; a `NotApplicable`/`Failed` message is a Provider Note and does not. A caveat qualifying a number that *was* collected is the first kind.
