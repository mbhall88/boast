...
- **Concurrency is bounded by a fixed constant (`MAX_CONCURRENT_HOSTS = 8`)**, independent of how many hosts or Identities are involved, so worker-thread count never scales with Manifest size.
