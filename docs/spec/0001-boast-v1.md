...
- **Default Provider set by Category.**
  ...
  - *Citations:* OpenAlex (count + FWCI + `citation_normalized_percentile`), Crossref (metadata), Europe PMC (life-sciences cross-check), Dimensions badge API (count + recent (last-two-calendar-year) count + FCR + RCR).
